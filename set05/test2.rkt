...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname test2) (read-case-sensitive #t) (teachpacks ((lib "arrow-gui.rkt" "teachpack" "htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "arrow-gui.rkt" "teachpack" "htdp")))))
(require "extras.rkt")
...
(define w0 (make-wall (make-posn 100 100)        
                      (make-posn 100 102)
                      V
                      #true #true))
...
(define (next-h-wall w ws)
  (compute-next-wall w ws H posn-x wall-y))

(define (next-v-wall w ws)
  (compute-next-wall w ws V posn-y wall-x))


(define (compute-next-wall w ws dir active-coor constant-coor)
  (local(
         (define ppd-ws (perpendicular dir ws))
         (define h1s (next-speed (wall-ac1 w) (wall-ac2 w)))
         (define h2s (next-speed (wall-ac2 w) (wall-ac1 w)))
         (define cs (map (lambda (p) (active-coor (wall-h1 p)))
                         ppd-ws))
